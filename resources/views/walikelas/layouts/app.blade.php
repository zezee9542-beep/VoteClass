...
<head>
    ...
    <title>@yield('title', 'Wali Kelas') - VoteClass</title>
    ...
    <style>
        ...
        .stat-card:hover {
            box-shadow: 0 8px 30px rgba(64, 88, 52, 0.10);
...
<!-- Sidebar -->
@include('walikelas.partials.sidebar')
    ...
    <header class="topbar py-4 px-6 border-b border-[#e8e0c8] bg-white">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-[#2f3d20] font-bold text-[18px] leading-none">
                    Selamat datang kembali, <span class="text-[#405834]">{{ Auth::user()->name }}</span> 👋
                </h1>
                <p class="text-[#aab2a3] text-[13px] font-medium mt-1.5">Kelola pemilihan ketua kelas {{ Auth::user()->class?->class_name }} dengan mudah, aman, dan transparan.</p>
            </div>
        </div>
        <div class="flex items-center gap-6">
            ...
            <button class="relative text-[#6e7568] hover:text-[#405834] transition-colors">
                ...
                <span class="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#6e9f45] text-white text-[10px] font-bold rounded-full flex items-center justify-center border-[1.5px] border-white">3</span>
            </button>
            <!-- Avatar -->
            <div class="flex items-center gap-3 cursor-pointer group pl-2 border-l border-[#e8e0c8]" onclick="openProfilePopup()" title="Pengaturan Profil">
                <div class="w-10 h-10 rounded-full bg-[#fdf9ef] flex items-center justify-center overflow-hidden border border-[#e8e0c8] hover:ring-2 hover:ring-[#d5b263]/50 transition-all">
                    <img id="topbarAvatarImg" src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&background=edf1e6&color=405834&bold=true' }}" alt="Wali Kelas" class="w-full h-full object-cover">
                </div>
                <div class="hidden sm:block text-left">
                    <p class="text-[#2f3d20] font-bold text-[14px] leading-none">{{ Auth::user()->name }}</p>
                    <p class="text-[#aab2a3] text-[12px] font-medium mt-1">Kelas {{ Auth::user()->class?->class_name }}</p>
                </div>
...
@include('partials.profile-popup', ['updateRoute' => route('walikelas.profile.update'), 'roleLabel' => 'Wali Kelas'])
